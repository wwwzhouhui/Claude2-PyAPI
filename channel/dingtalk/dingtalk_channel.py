...
class DingTalkHandler():
    ...
    def notify_dingtalk_webhook(self, data):
        ...
        try:
            logger.info("[DingTalk] url={}".format(str(notify_url)))
            r = requests.post(notify_url, json=data)
            reply = r.json()
            logger.info("[DingTalk] reply={}".format(str(reply)))
    ...
    def notify_dingtalk(self, data, reply_json):
        ...
        try:
            ...
            resp = r.json()
            logger.info("[DingTalk] response={}".format(str(resp)))
        except Exception as e:
            logger.error(e)
...
class DingTalkChannel(Channel):
    def __init__(self):
        self.host = conf().get('dingtalk_host')
        self.port = conf().get('dingtalk_port')
